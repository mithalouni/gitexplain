...
function startServer(): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    proc.stdout.on('data', (chunk: Buffer) => {
      ...
      if (m) done(null, `http://localhost:${m[1]}`);
    });
...
async function hasKey(): Promise<boolean> {
  ...
  try {
    ...
    const body = (await r.json()) as { hasKey?: boolean };
    return !!body.hasKey;
  } catch {
    return false;
  }
...
async function createLesson(args: unknown): Promise<{ viewUrl: string; lessonId: string }> {
  if (!baseUrl) throw new Error('GitExplain server not ready');
  ...
  tools: [
    {
      name: 'create_lesson',
      description:
        'Create a narrated slide-video lesson that plays in the browser with synced ElevenLabs voice. Use when the user asks to "explain", "walk through", "teach", or "make a lesson about" code or a concept. ' +
      ...
      inputSchema: {
        ...
        properties: {
          ...
          slides: {
            ...
            minItems: 1,
            description:
              'Ordered array of slide objects. Each slide has { type, narration, data }. Start with a "title" slide and end with a "summary" slide. See the slide_types_guide prompt for the exact data schema per type.',
            items: {
              type: 'object',
              ...
              properties: {
                type: {
                  type: 'string',
                  description:
                    'One of: title, bullets, code, codeCompare, fileTree, flowDiagram, sequenceDiagram, architecture, callout, definition, twoColumn, stats, terminal, steps, timeline, warning, quiz, summary, image, comparison.',
                },
                narration: {
                  type: 'string',
                  description:
                    'Voice-over text spoken by ElevenLabs. Conversational, 1-3 sentences. Reinforces the visual — do not just read it aloud.',
                },
                data: {
                  type: 'object',
...
mcp.setRequestHandler(CallToolRequestSchema, async (req) => {
  ...
  const args = req.params.arguments as { title?: string; slides?: unknown[] };
        ...
        content: [
          {
            type: 'text',
            text:
              `Lesson created (${args.slides.length} slides), but no ElevenLabs key is saved yet so narration won't play.\n\n` +
  ...
  prompts: [
    {
      ...
      description:
        'Full schema reference for all 20 GitExplain slide types. Fetch this before calling create_lesson so you know exactly what data shape each slide type expects.',
    },
  ],
}));
